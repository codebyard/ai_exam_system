...
export const useExamStore = create<ExamStore>((set, get) => ({
  ...
  resumeSession: () => {
    ...
    if (session) {
      set({
        session: { ...session, isPaused: false, isTimerRunning: session.mode === 'exam' }
      });
    }
  },
  ...
  getQuestionState: (questionId) => {
    ...
    return initialQuestionState(questionId);
  },
  ...
  getSessionSummary: () => {
    ...
    const answered = states.filter(s => s.isAnswered && !s.isMarkedForReview).length;
    const marked = states.filter(s => s.isMarkedForReview).length;
    const total = session.questions.length;
    const unanswered = total - answered - marked;
